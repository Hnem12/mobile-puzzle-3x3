...
function History() {
  ...
  return <section><h1>Lịch sử chơi</h1><div className="filters"><input placeholder="Tìm tên..." value={q} onChange={e => setQ(e.target.value)} /><button type="button" onClick={exportExcel}><Download size={18} /> Xuất Excel</button></div><p>Tổng: <b>{rows.length}</b> bản ghi</p><div style={{ maxHeight: 'calc(100vh - 250px)', overflowY: 'auto', overflowX: 'auto' }}><table><thead><tr><th>Người dùng</th><th>Số điện thoại</th><th>Cấp</th><th>Kết quả</th><th>Điểm</th><th>Thời gian (s)</th><th>Bước</th><th>Ngày chơi</th></tr></thead><tbody>{rows.map(r => <tr key={r._id}><td>{r.userId?.fullName}</td><td>{r.userId?.phone}</td><td>{r.levelId?.name}</td><td><span className={r.result === "WIN" ? "ok" : "bad"}>{r.result}</span></td><td>{r.score}</td><td>{r.duration}</td><td>{r.moves}</td><td>{r.playedAt?.slice(0, 10)}</td></tr>)}</tbody></table></div></section>;
}
function Users() {
  const [rows, setRows] = useState<any[]>([]); const [q, setQ] = useState("");
  useEffect(() => { 
